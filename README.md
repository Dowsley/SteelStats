...
- ```vue serve``` 👉 Para compilar e colocar no ar (hot-reload). Comando que será usado regularmente na fase de development.
- ```vue ui``` 👉 Similar ao serve, mas com uma GUI para o desenvolvedor.
- ```vue build``` 👉 Para compilar e buildar o app para produção. Não será usado até o final do projeto.
